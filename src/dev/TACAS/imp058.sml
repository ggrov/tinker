...
val ¶_not_binding_thm = tac_proof(([],¬µ p:BOOL · (¶ x:'a· p) = p®),
	rewrite_tac[]);
val µ_not_binding_thm = tac_proof(([],¬µ p:BOOL · (µ x:'a· p) = p®),
	rewrite_tac[]);
val uncurry_curry_thm = (
push_goal([],¬µ f f' · (Uncurry(Curry f) = f) ± (Curry(Uncurry f') = f')®);
a(rewrite_tac[ext_thm, pair_clauses]);
pop_thm());
val ¶_uncurry_thm = (
push_goal([],¬µ P · (¶ f · P f) ¤ (¶ f' · P(Uncurry f'))®);
a(REPEAT strip_tac);
(* *** Goal "1" *** *)
...
local
	val thm = tac_proof(([],¬µ t:BOOL · pp'TS t ¤ t®),
...
val rec all_simple_¶_µ_conv1 :CONV = (fn tm =>
	(simple_¶_µ_conv1 THEN_TRY_C
	(SIMPLE_BINDER_C all_simple_¶_µ_conv1)) tm
);
local
	val ´_thm = tac_proof(([],¬µ x · x ´ x®),REPEAT strip_tac);
in
...
local
	...
	val rwc = local_pure_rewrite_conv
		...
		µ_rewrite_thm, eq_rewrite_thm, ´_rewrite_thm,
			 ³_rewrite_thm];
...
local
	...
	let	val (x',args) = strip_app tm1;
	in
		list_mk_simple_Ì(gen_vars(map type_of args) [tm2], tm2)
	end);
in
...
in
fun opt_list_simple_µ_elim (alist:(TERM * TERM) list) (thm:THM): THM = (
...
(let	val base_xs = (hd stripped_usages) to (tryupto -1);
	...
	val s3 = conv_rule(RAND_C(LAST_SIMPLE_¶_C 
		(all_simple_¶_µ_conv1 THEN_TRY_C redundant_simple_µ_conv))) s2;
in
	s3
end)
end);
...
local
	val aux_thm = tac_proof(([],¬µ p q · (p ¤ (q ² pp'TS p)) ¤ (q ´ p)®),
...
let	val (f,bdy) = dest_simple_¶ tm;
	...
	val new_rthm' = conv_rule (ONCE_MAP_C 
		(COND_C (fn tm => is_simple_Ì tm andalso
			fst(dest_var(fst(dest_simple_Ì tm))) = ftname)
		(simple_Á_conv fname)
		(fail_conv))) new_rthm;
	...
	in
		list_mk_simple_Ì (gvs,subst (combine gvs rnargs) conj')
	end);
	val preds = map get_pred (combine conjs conjst);
	val snd_thm = conv_rule(TRY_C all_simple_Â_conv)
		 (list_simple_µ_elim preds new_rthm');
	val fst_thm = TRY_C all_simple_Â_conv tm;
in
...
let	fun move_in_conv ltm = (case (dest_term ltm) of
		...
		| Dµ (vs,_) => (
			...
			AND_OR_C
			(µ_±_conv THEN_TRY_C move_in_conv)
			ORELSE_C
			(ALL_SIMPLE_µ_C move_in_conv 
			THEN_TRY_C redundant_simple_µ_conv)) ltm
