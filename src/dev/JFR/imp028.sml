...
val ´_thm = save_thm("´_thm", tac_proof( ([], ¬µa b·(a ´ b) ¤ (³a ² b)®),
	REPEAT strip_tac
...
val ³_³_thm = save_thm("³_³_thm", tac_proof( ([], ¬µa·³ ³a ¤ a®),
		REPEAT strip_tac THEN_LIST
	[accept_tac (³_³_elim (asm_rule ¬³³a®)),
	 accept_tac (³_³_intro (asm_rule ¬a:BOOL®))]));
val ³_²_thm = save_thm("³_²_thm", tac_proof( ([], ¬µa b·³ (a ² b) ¤ (³a ± ³b)®),
...
val ³_´_thm = save_thm("³_´_thm", tac_proof( ([], ¬µa b·³(a ´ b) ¤ (a ± ³b)®),
	conv_tac(TOP_MAP_C(FIRST_C (map simple_eq_match_conv
		[´_thm, ³_²_thm, ³_³_thm] )))
		THEN REPEAT strip_tac));
val ³_¤_thm = (* ô µ a b · ³ (a ¤ b) ¤ a ± ³ b ² b ± ³ a  *)
		save_thm("³_¤_thm",
		list_simple_µ_intro[¬a:BOOL®, ¬b:BOOL®]
...
val ³_if_thm = (* ô µ a b c· ³(if a then b else c) ¤ (if a then ³b else ³c)  *)
	save_thm("³_if_thm", 
...
val ²_comm_thm : THM = tac_proof(([], ¬µa b·a ² b ¤ b ² a®), 
	REPEAT strip_tac);
